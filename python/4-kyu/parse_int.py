...
def parse_int(string):
    ...
    while i < len(numbers):
        ...
        if "-" in numbers[i]:
            ...
            final_result.append(translate_number(numbers[i]))
        if i < len(numbers)-1:
            if numbers[i+1] == "hundred" or numbers[i+1] == "thousand" or numbers[i+1] == "million":
                num = final_result.pop() * translate_number(numbers[i+1])
                final_result.append(num)
                i += 1
        i += 1
